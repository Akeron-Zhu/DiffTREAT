    ...
    def __init__(self, el=None):
        ...
        if el is not None:
            ...
            for bin in el.findall('bin'):
                self.bins.append((float(bin.get("start")), float(
                    bin.get("width")), int(bin.get("count"))))
...
class Flow(object):
    ...
    def __init__(self, flow_el):
        ...
        tx_duration = float(long(flow_el.get('timeLastTxPacket')[
                            :-4]) - long(flow_el.get('timeFirstTxPacket')[:-4]))*1e-9
        rx_duration = float(long(flow_el.get('timeLastRxPacket')[
                            :-4]) - long(flow_el.get('timeFirstRxPacket')[:-4]))*1e-9
        fct = float(long(flow_el.get('timeLastRxPacket')[
                    :-4]) - long(flow_el.get('timeFirstTxPacket')[:-4]))*1e-9
        ...
        if rxPackets:
            self.hopCount = float(flow_el.get(
                'timesForwarded')) / rxPackets + 1
        else:
            self.hopCount = -1000
        if rxPackets:
            self.delayMean = float(flow_el.get('delaySum')[
                                   :-4]) / rxPackets * 1e-9
            ...
            for stats in probe_elem.findall("FlowStats"):
                ...
                if s.packets > 0:
                    s.delayFromFirstProbe = parse_time_ns(
                        stats.get('delayFromFirstProbeSum')) / float(s.packets)
                else:
...
def parse(fileName, id):
    ...
    if large_flow_count == 0:
        ...
        print "Large Flow Avg FCT: %.4f" % (
            large_flow_total_fct / large_flow_count)

    if small_flow_count == 0:
        print "No small flows"
    else:
        print "Small Flow Avg FCT: %.4f" % (
            small_flow_total_fct / small_flow_count)
...
def main(argv):
    ...
    for flowKey in baseMap.iterkeys():
        ...
        if flowKey not in compareMap:
            total_not_exist += 1
            print "Flow (%s/%s --> %s/%s) does not exist, the size should be %i" % (flowKey.sourceAddress,
                                                                                    flowKey.sourcePort, flowKey.destinationAddress, flowKey.destinationPort, baseSize)

    for flowKey in baseMap.iterkeys():
        baseSize = baseMap[flowKey]
        if flowKey in compareMap and baseSize > compareMap[flowKey]:
            total_wrong_size += 1
            print "Flow (%s/%s --> %s/%s) is in wrong size, the size should be %i but now it is: %i" % (flowKey.sourceAddress,
                                                                                                        flowKey.sourcePort, flowKey.destinationAddress, flowKey.destinationPort, baseSize, compareMap[flowKey])
    print "Not exist flows: %i, Wrong size flows: %i" % (
        total_not_exist, total_wrong_size)
