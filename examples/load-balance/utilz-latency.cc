...
void install_applications(int fromLeafId, NodeContainer servers, double requestRate, struct cdf_table *cdfTable,
                          long &flowCount, long &totalFlowSize, int PER_LEAF_SERVER_COUNT, int LEAF_COUNT, double START_TIME,
                          double END_TIME, double FLOW_LAUNCH_END_TIME, uint32_t applicationPauseThresh, uint32_t applicationPauseTime)
{
    ...
    for (int i = 0; i < PER_LEAF_SERVER_COUNT; i++)
    {
        ...
        //得到服务器的坐标
        int fromServerIndex = fromLeafId * PER_LEAF_SERVER_COUNT + i;
        //得到这个服务器开始发送流的时间
        ...
        while (startTime < FLOW_LAUNCH_END_TIME)
        {
            ...
            while (destServerIndex >= fromLeafId * PER_LEAF_SERVER_COUNT && destServerIndex < fromLeafId * PER_LEAF_SERVER_COUNT + PER_LEAF_SERVER_COUNT)
            {
            ...
            /*NS_LOG_INFO ("\tFlow from server: " << (((servers.Get(fromServerIndex))->GetObject<Ipv4>())->GetAddress(1,0)).GetLocal() << " to server: "
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < LEAF_COUNT; i++)
    {
        ...
        for (int j = 0; j < PER_LEAF_SERVER_COUNT; j++)
        {
            ...
            if (runMode == CONGA || runMode == CONGA_FLOW || runMode == CONGA_ECMP)
            {
                ...
                staticRoutingHelper.GetStaticRouting(servers.Get(serverIndex)->GetObject<Ipv4>())->AddNetworkRouteTo(Ipv4Address("0.0.0.0"), 
                                                                                                Ipv4Mask("0.0.0.0"), netDeviceContainer.Get(1)->GetIfIndex());
            ...
            if (runMode == DRILL)
            {
                ...
                drillRoutingHelper.GetDrillRouting(leaves.Get(i)->GetObject<Ipv4>())->AddRoute(interfaceContainer.GetAddress(1), 
            ...
            for (int l = 0; l < LINK_COUNT; l++)
            {
                ...
                if (runMode == DRILL)
                {
                    ...
                    for (int k = 0; k < LEAF_COUNT; k++)
                    {
                        ...
                        {
                            //配置路由表，意思是叶结点i到其它叶结点都可以通过现在这个骨干结点到达，所以可以将包发送这个core节点的端口
                            drillRoutingHelper.GetDrillRouting(leaves.Get(i)->GetObject<Ipv4>())->AddRoute(leafNetworks[k], 
                                                                                                    Ipv4Mask("255.255.255.0"), 
                                                                                                    netDeviceContainer.Get(0)->GetIfIndex());
                        }
                    }
    ...
    for (int fromLeafId = 0; fromLeafId < LEAF_COUNT-1; fromLeafId++)
    {
        install_applications(fromLeafId, servers, requestRate, cdfTable, flowCount, totalFlowSize, PER_LEAF_SERVER_COUNT, LEAF_COUNT, START_TIME, END_TIME, FLOW_LAUNCH_END_TIME, applicationPauseThresh, applicationPauseTime);
    }
...
//设置输出的文件名称，第一个是runningID,   
    ...
    {
        NS_LOG_INFO("统计数与设置数不符全，请重新运行仿真");
    }
    else
    {
        std::stringstream doubleToStr;
        doubleToStr<<"./t.sh "<<flowMonitorFilename.str().c_str()<<" "<<load<<" "<<id;
