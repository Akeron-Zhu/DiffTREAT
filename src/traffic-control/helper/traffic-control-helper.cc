  ...
  for (int i = m_queueDiscFactory.size () - 1; i >= 0; i--)
    {
      ...
      /*********************************************Add by myself*********************/
      Ptr<PrioQueueDisc> p=DynamicCast<PrioQueueDisc>(q);
      if(p) 
      {
        //p->SetCache(d->GetNode()->GetCache());
        //std::cout<<"DiscId is "<<m_prioDiscId<<std::endl;
       // p->SetDiscId(m_prioDiscId++);//*/
        Ptr<Cache> cache = d->GetNode()->GetCache();
	if(cache) cache->AddQueueDisc(p);
      }
      /*********************************************Add by myself*********************/
