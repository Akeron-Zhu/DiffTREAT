...
void QueueDisc::SetNetDevice(Ptr<NetDevice> device)
{
...
void QueueDisc::AddInternalQueue(Ptr<Queue> queue)
{
  NS_LOG_FUNCTION(this);
...
/**********************************************************************/
void QueueDisc::EnqueueDecache(Ptr<QueueDiscItem> item)
{
  NS_LOG_FUNCTION(this << item);

  m_nPackets++;
  m_nBytes += item->GetPacketSize();
  NS_LOG_LOGIC("Cache->Queue (p)");
}

void QueueDisc::DequeueEncache(Ptr<QueueDiscItem> item)
{
  NS_LOG_FUNCTION(this);
  if (item != 0)
  {
