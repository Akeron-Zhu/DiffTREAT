...
public:
  ...
  void EnqueueDecache (Ptr<QueueDiscItem> item);
  void DequeueEncache (Ptr<QueueDiscItem> item);
...
private:
  ...
  virtual bool DoEnqueue (uint32_t band,Ptr<QueueDiscItem> item) = 0;
  virtual Ptr<QueueDiscItem> DoDequeue (uint32_t band) = 0;
