...
public:
  ...
  /******************************************************************************/
  void SetNewArgu(bool cacheable, uint32_t retxThre);
  /******************************************************************************/
...
protected:
  ...
  /******************************************************************************/
  void SendUrgePacket(uint32_t urgeNum);
  /******************************************************************************/
protected:
  // Counters and events
  EventId m_retxEvent;         //!< Retransmission event
  EventId m_urgePktEvent;      //Add by myself
  ...
  RttHistory_t m_history;      //!< List of sent packet 发送包的列表

  /******************************************************************************/

  int m_retransmit_time;
  int m_retranDupack;
  int m_retranRec;
  int m_retranTimeout;
  int m_retranTime;
  uint32_t m_urgeSendNum;
  uint32_t m_flowId;
  bool enablePrt;
  uint32_t m_urgeNum;
  bool m_cacheable;
  bool m_enableUrgeSend;
  uint32_t m_tmpRetxThre;
  Time m_tmpMinRto;
  /*******************************************************************/
  ...
  double m_msl;                     //!< Max segment lifetime 报文生存最大时间
  ...
  bool m_isFirstPartialAck; //!< First partial ACK during RECOVERY 是否为部分ACK的第一个包
