...
BulkSendApplication::GetTypeId(void)
{
  static TypeId tid = TypeId("ns3::BulkSendApplication")
                          ...
                          .AddAttribute("EnableSizeRank", "Enable SizeRank",
                                        ...
                                        MakeBooleanChecker())
                          .AddAttribute("NTcp", "Enable Cacheable",
                                        BooleanValue(false),
                                        MakeBooleanAccessor(&BulkSendApplication::m_isNTcp),
                                        MakeBooleanChecker())
...
void BulkSendApplication::SendData(void)
{
  ...
  double thre[3][7] = {1898000, 2920000, 2336000, 2628000, 2920000, 3212000, 3358000,
                       1606000, 17762360, 2117000, 17762360, 18980000, 30952000, 37960000,
                        3333000,6666000};
  ...
  while (m_maxBytes == 0 || m_totBytes < m_maxBytes)
  {
    ...
    if (m_enableSizeRank)
    {
      if (m_totBytes >= thre[inx][1])
      {
        m_sizeRank = 2;
      }
      else if (m_totBytes >= thre[inx][0])
      {
        m_sizeRank = 1;
      }
      else
